...
- [x] **UserConsent Model**: Implement a new SQLAlchemy model `UserConsent` in `src/api/fastapi/main.py`:
...
- [x] **GET /v1/me/profile**: Return current user profile (from Keycloak JWT) and their consent status:
...
- [x] **PUT /v1/me/consents**: Update consent preferences:
...
- [x] **Access Guard**: Prevent any authenticated action (except Profile GET) if `deletion_pending_at` is set.

### 2. Data Portability (Right to Portability)
- [x] **GET /v1/me/export-data**: Generate and return a ZIP archive containing all data associated with the user:
...
- [x] **Streaming Response**: Ensure the export is streamed to avoid memory issues with large datasets. Use a Redis lock `lock:export:{user_id}` to prevent concurrent exports or deletions.

### 3. Right to be Forgotten (Deletion & Anonymization)
- [x] **DELETE /v1/me/account**: Trigger a "Right to be Forgotten" workflow:
...
- [x] **Admin Purge Sync**: Reuse `DELETE /v1/media/purge/{object_key}` logic if needed for specific files associated solely with the user.

### 4. UI: Profile Center (Azure Flow)
- [x] **Profile Page**: Create a new page `/profile` in the frontend:
...
- [x] **Navigation**: Add a "Profile" entry in the AppShell avatar menu.

## Tasks / Subtasks

- [x] **Task 1: Backend Models & Endpoints**
  - [x] Add `UserConsent` table to `main.py` with `deletion_pending_at`.
  - [x] Implement `GET /v1/me/profile`.
  - [x] Implement `PUT /v1/me/consents` with immediate ML purge logic.
  - [x] Implement `GET /v1/me/export-data` (Zip streaming).
- [x] **Task 2: Deletion & Anonymization Logic**
  - [x] Implement `DELETE /v1/me/account` (Grace period vs Immediate).
  - [x] Implement anonymization routines for `assignments` and `audit_logs`.
  - [x] Add integration test for account deletion and anonymization.
- [x] **Task 3: Frontend UI**
  - [x] Build the `/profile` page using `AzureTable` or standard components.
  - [x] Integrate with the new FastAPI endpoints.
  - [x] Add confirmation modals for critical actions (consents, deletion).
