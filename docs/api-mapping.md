...
- **`POST /v1/editor/ticket`** (Story 5.2 — implémenté)
  ...
  - **URL WSS (dev Compose) :** `ws://localhost:1234` (port exposé du service `hocuspocus` dans `src/compose.yml`). Le **nom de document** (room) est la chaîne **`audio_files.id`** (`document_id` / `audio_id` dans l’URL de l’éditeur), ex. room `"42"` pour `?audio_id=42`.
